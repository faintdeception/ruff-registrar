...
public class AccountHoldersController : ControllerBase
{
    ...
    public async Task<ActionResult<CreateAccountHolderResponse>> CreateAccountHolder(CreateAccountHolderDto createDto)
    {
        ...
        try
        {
            ...
            _logger.LogInformation("Created Keycloak user with ID: {KeycloakId}", keycloakUserResponse.UserId);
            ...
            // Log the account creation for security audit (without password)
            _logger.LogInformation("Created account for {Email} by admin {AdminEmail}. User must change password: {MustChangePassword}", 
